...
def get_subroots(root: str, max_depth: int = 1) -> list[str]:
    ...
    for dirpath, dirs, _ in tqdm(os.walk(root), desc="Walking directory tree for subroots", leave=False):
...
def dump_all_json_paths(
    ...
    output_files = [task[1] for task in tasks]
    return output_files


def dump_csv(txt_path: str, ignore_partition_key=True, output_folder: str = "csv_results") -> str:
    """
...
def dump_all_csv(json_path: str | list[str], output_folder: str = "csv_results") -> bool:
    """
...
def load_experiment_data(results_folder: str):
    ...
    time.sleep(2)
    df = get_combined_dataframe(csv_paths)
    return df
